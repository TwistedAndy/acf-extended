	...
	new acf.Model({
		...
		init: function($select, options, data, field, instance) {
			...
			if ($select.data('select2')) {
				...
				$select.data('select2').$dropdown.addClass('select2-dropdown-acf').addClass('select2-dropdown-acf-field-' + field.get('name')).addClass('select2-dropdown-acf-field-' + field.get('key'));
...
(function($) {
	...
	var ACFE_Slug = acf.Field.extend({
		...
		onInput: function(e, $el) {
			...
			$el.val($el.val().toLowerCase().replace(/\s+/g, '-') // Replace spaces with -
		...
		onFocusOut: function(e, $el) {
			...
			$el.val($el.val().toLowerCase().replace(/-+$/, '') // Trim - from end of text
