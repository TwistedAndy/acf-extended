...
function acfe_remove_filter($tag = '', $function = '', $priority = 10) {
	...
	if (isset($acf_instances[$class])) {
		return remove_filter($tag, [$acf_instances[$class], $method], $priority);
	}
	...
	if ($field_type) {
		return remove_filter($tag, [$field_type, $method], $priority);
	}
...
function acfe_replace_filter($tag = '', $function_to_remove = '', $function_to_add = '', $priority = 10, $accepted_args = 1) {
	...
	if (isset($acf_instances[$class]) && is_callable([$acf_instances[$class], $method])) {
		return add_filter($tag, [$acf_instances[$class], $method], $priority, $accepted_args);
	}

	// check acf field types
	$field_type = acf_get_field_type($class);

	if ($field_type && is_callable([$field_type, $method])) {
		return add_filter($tag, [$field_type, $method], $priority, $accepted_args);
	}
