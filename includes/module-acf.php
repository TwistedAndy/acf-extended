...
class acfe_module_acf {
	...
	function __construct() {
		...
		add_action('acf/validate_save_post', [$this, 'after_validate_save_post']);
		add_filter('acf/pre_load_value', [$this, 'pre_load_value'], 10, 3);
		add_action('acf/save_post', [$this, 'save_post'], 1);
		add_action('acf/include_admin_tools', [$this, 'include_admin_tools'], 15);
		add_action('acf/include_admin_tools', [$this, 'include_admin_tools_sort'], 99);
	...
	function pre_load_value($null, $post_id, $field) {
		...
		if (empty($this->values)) {
			...
			// remove unused keys
			acf_extract_vars($item, ['ID', '_valid']);
			...
			$acf = acfe_prefix_array_keys($item, 'field_', ['acf_fc_layout']);
