...
class acfe_field_flexible_content_actions {
	...
	function __construct() {
		...
		// Hooks
		add_filter('acfe/flexible/defaults_field', [$this, 'defaults_field'], 6);
		add_filter('acfe/flexible/render_field_settings', [$this, 'render_field_settings'], 6);

		add_filter('acfe/flexible/validate_field', [$this, 'validate_actions']);
		add_filter('acfe/flexible/wrapper_attributes', [$this, 'wrapper_attributes'], 10, 2);
		add_filter('acfe/flexible/load_fields', [$this, 'load_fields'], 10, 2);
		add_filter('acfe/flexible/layouts/div', [$this, 'layout_div'], 10, 6);
		add_filter('acfe/flexible/prepare_layout', [$this, 'prepare_layout'], 10, 5);
		add_filter('acfe/flexible/layouts/icons', [$this, 'layout_icons'], 11, 3);
		add_filter('acfe/flexible/secondary_actions', [$this, 'secondary_actions'], 10, 2);

		add_filter('acf/fields/flexible_content/layout_attrs', [$this, 'layout_attrs'], 5, 4);
	...
	function prepare_layout($layout, $field, $i, $value, $prefix) {
		...
		if (in_array('title', $actions)) {
			...
			if ($sub_field) {
				...
				foreach ([
					...
					'disabled',
					'required',
					'data-acfe-flexible-control-title-input'
				] as $k) {
		...
		if (in_array('toggle', $actions)) {
			...
			if ($sub_field) {
				...
				$input_attrs = [];
				foreach (['type', 'id', 'class', 'name', 'value'] as $k) {
