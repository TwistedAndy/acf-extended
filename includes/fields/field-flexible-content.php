...
class acfe_field_flexible_content extends acfe_field_extend {
	...
	function validate_field($field) {
		...
		// default filter
		$defaults_field = apply_filters('acfe/flexible/defaults_field', []);
	...
	function validate_layout($layout, $field) {
		...
		// defaults filter
		$defaults_layout = apply_filters('acfe/flexible/defaults_layout', [], $field);
	...
	function prepare_field($field) {
		...
		foreach ($field['layouts'] as &$layout) {
			...
			if (!empty($atts)) {
				$layout['label'] .= '<span ' . acf_esc_atts($atts) . '>';
			}
			$layout['label'] .= $label;
			if (!empty($atts)) {
				$layout['label'] .= '</span>';
	...
	function render_actions($field, $position = 'bottom') {
		...
		// filter (with variations)
		$wrapper = ['class' => "acf-actions acf-fc-{$position}-actions"];
	...
	function get_layout_handle_elements($layout, $field, $i, $value, $prefix) {
		...
		// filter (with variations)
		$attrs = ['class' => 'acf-fc-layout-title'];
