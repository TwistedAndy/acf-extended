...
class acfe_compatibility_acf_64 {
	...
	function __construct() {
		 ...
		 */
		acfe_replace_action('wp_restore_post_revision', [acf()->revisions, 'wp_restore_post_revision'], [$this, 'wp_restore_post_revision'], 10, 2);
		...
		if (version_compare(get_bloginfo('version'), '6.4', '>=')) {
			...
			acfe_replace_action('_wp_put_post_revision', [acf()->revisions, 'maybe_save_revision'], [$this, 'maybe_save_revision'], 10, 2);

			acfe_replace_filter('wp_save_post_revision_post_has_changed', [acf()->revisions, 'check_acf_fields_have_changed'], [
				$this,
	...
	function check_acf_fields_have_changed($post_has_changed, $last_revision, $post) {
		...
		if (method_exists(acf()->revisions, 'check_acf_fields_have_changed')) {
	...
	function maybe_save_revision($revision_id, $post_id) {
		...
		// no performance mode enabled on this post
		// fallback to ACF native method
		if (method_exists(acf()->revisions, 'maybe_save_revision')) {
	...
	function wp_restore_post_revision($post_id, $revision_id) {
		...
		// no performance mode enabled on this post
		// fallback to ACF native method
		if (method_exists(acf()->revisions, 'wp_restore_post_revision')) {
