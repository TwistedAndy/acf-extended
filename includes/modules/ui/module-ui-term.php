...
class acfe_enhanced_ui_term extends acfe_enhanced_ui {
	...
	function initialize() {
		...
		// load
		add_action('acfe/load_term', [$this, 'load_term']);
		add_action('acfe/load_terms', [$this, 'load_terms']);
	...
	function load_term($taxonomy) {
		...
		// hooks
		add_action('admin_enqueue_scripts', [$this, 'term_enqueue_scripts'], 15); // must be priority 15
		add_action('admin_footer', [$this, 'term_footer']);
	...
	function load_terms($taxonomy) {
		...
		// hooks
		add_action('admin_footer', [$this, 'terms_footer']);
	...
	function add_term_meta_boxes($taxonomy, $term) {
		...
		// Sidebar submit
		add_meta_box('submitdiv', __('Edit'), [$this, 'render_metabox_submit'], $screen, 'side', 'high', 'term');
	...
	function term_enqueue_scripts() {
		...
		acfe_remove_action("{$this->taxonomy}_edit_form", ['acf_form_taxonomy', 'edit_term']);
