...
class ACFE_AutoSync {
	...
	function __construct() {
		...
		// Json Save
		add_action('acf/update_field_group', [$this, 'pre_update_field_group_json'], 9);
		add_action('acf/untrash_field_group', [$this, 'pre_update_field_group_json'], 9);

		add_action('acf/update_field_group', [$this, 'post_update_field_group_json'], 11);
		add_action('acf/untrash_field_group', [$this, 'post_update_field_group_json'], 11);
	...
	function pre_update_field_group_json($field_group) {
		...
		if (!acfe_has_json_sync($field_group)) {
			return add_filter('acf/settings/json', [$this, '__return_false']);
		}
	...
	function post_update_field_group_json($field_group) {
		...
		if (!acfe_has_json_sync($field_group)) {
			return remove_filter('acf/settings/json', [$this, '__return_false']);
		}
...
function acfe_has_php_sync($item) {
	return in_array('php', (array) acf_maybe_get($item, 'acfe_autosync', []));
}
