...
if (!class_exists('acfe_dynamic_forms_helpers')):
	...
	class acfe_dynamic_forms_helpers {
		...
		function get_fields_choices($deep = false, $original_field = []) {
		...
		function map_fields_values(&$data = [], $array = []) {
		...
		function map_field_value($content, $post_id = 0, $form = []) {
		...
		function map_field_value_load($content, $post_id = 0, $form = []) {
...
function acfe_form_map_current($content, $post_id = 0, $form = []) {
...
function acfe_form_map_action($content) {
	...
	if (preg_match_all('/{action:(.*?)}/', $content, $matches)) {
		...
		foreach ($matches[1] as $i => $name) {
			...
			$value = false;
			$last_action = get_query_var('acfe_form_actions', []);
...
function acfe_form_map_vs_fields($map, $fields, $post_id = 0, $form = []) {
