...
class acfe_form_front {
	...
	function __construct() {
		...
		// shortcode
		add_shortcode('acfe_form', [$this, 'render_shortcode']);

		// save
		add_action('acf/validate_save_post', [$this, 'validate_save_post'], 1);
		add_action('wp', [$this, 'save_post']);

		// ajax
		add_action('wp_ajax_acfe/form/shortcode', [$this, 'ajax_shortcode'], 20);
		add_action('wp_ajax_nopriv_acfe/form/shortcode', [$this, 'ajax_shortcode'], 20);
	...
	function save_post() {
		...
		// ACF
		$_POST['acf'] = acf_maybe_get_POST('acf', []);
	...
	function form_wrapper($args, $open = true) {
		...
		if ($open) {
			...
			if ($is_preview) {
				remove_filter('acf/prepare_field', [$this, 'disable_fields']);
			}
	...
	function render_fields($args, $fields) {
		...
		if ($args['post_field_groups']) {
			...
			$post_field_groups = acf_get_field_groups([
	...
	function render_shortcode($atts) {
		...
		foreach (array_keys($atts) as $key) {
			...
			foreach (['form_attributes_', 'fields_attributes_'] as $allowed) {
