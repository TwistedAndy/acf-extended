...
class acfe_form_term {
	...
	function __construct() {
		...
		add_filter('acfe/form/load/term', [$this, 'load'], 10, 3);
		add_action('acfe/form/make/term', [$this, 'make'], 10, 3);
		add_action('acfe/form/submit/term', [$this, 'submit'], 10, 5);

		/*
		 * Admin
		 */
		add_filter('acf/prepare_field/name=acfe_form_term_save_meta', [$helpers, 'map_fields']);
		add_filter('acf/prepare_field/name=acfe_form_term_load_meta', [$helpers, 'map_fields']);

		add_filter('acf/prepare_field/name=acfe_form_term_save_target', [$helpers, 'map_fields_deep']);
		add_filter('acf/prepare_field/name=acfe_form_term_load_source', [$helpers, 'map_fields_deep']);

		add_filter('acf/prepare_field/name=acfe_form_term_save_name', [$helpers, 'map_fields_deep']);
		add_filter('acf/prepare_field/name=acfe_form_term_save_slug', [$helpers, 'map_fields_deep']);
		add_filter('acf/prepare_field/name=acfe_form_term_save_taxonomy', [$helpers, 'map_fields_deep']);
		add_filter('acf/prepare_field/name=acfe_form_term_save_parent', [$helpers, 'map_fields_deep']);
		add_filter('acf/prepare_field/name=acfe_form_term_save_description', [$helpers, 'map_fields_deep']);

		add_filter('acf/prepare_field/name=acfe_form_term_map_name', [$helpers, 'map_fields_deep_no_custom']);
		add_filter('acf/prepare_field/name=acfe_form_term_map_slug', [$helpers, 'map_fields_deep_no_custom']);
		add_filter('acf/prepare_field/name=acfe_form_term_map_taxonomy', [$helpers, 'map_fields_deep_no_custom']);
		add_filter('acf/prepare_field/name=acfe_form_term_map_parent', [$helpers, 'map_fields_deep_no_custom']);
		add_filter('acf/prepare_field/name=acfe_form_term_map_description', [$helpers, 'map_fields_deep_no_custom']);

		add_filter('acf/prepare_field/name=acfe_form_term_save_target', [$this, 'prepare_choices'], 5);
		add_filter('acf/prepare_field/name=acfe_form_term_load_source', [$this, 'prepare_choices'], 5);
		add_filter('acf/prepare_field/name=acfe_form_term_save_parent', [$this, 'prepare_choices'], 5);
	...
	function submit($_term_id, $term_action, $args, $form, $action) {
		...
		// Deprecated
		$term_object = apply_filters_deprecated("acfe/form/query_var/term", [
			$term_object,
			$_term_id,
			$term_action,
			$args,
			$form,
			$action
		], '0.8.7.5', "acfe/form/output/term");
		$term_object = apply_filters_deprecated("acfe/form/query_var/term/form={$form_name}", [
			$term_object,
			$_term_id,
			$term_action,
			$args,
			$form,
			$action
		], '0.8.7.5', "acfe/form/output/term/form={$form_name}");
		$term_object = apply_filters_deprecated("acfe/form/query_var/term/action={$action}", [
			$term_object,
			$_term_id,
			$term_action,
			$args,
			$form,
			$action
		], '0.8.7.5', "acfe/form/output/term/action={$action}");
