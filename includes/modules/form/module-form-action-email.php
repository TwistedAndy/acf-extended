...
class acfe_form_email {
	...
	function __construct() {
		...
		add_action('acfe/form/make/email', [$this, 'make'], 10, 3);
		add_action('acfe/form/submit/email', [$this, 'submit'], 10, 3);

		/*
		 * Admin
		 */
		add_filter('acf/prepare_field/name=acfe_form_email_file', [$helpers, 'map_fields_deep']);
	...
	function make($form, $current_post_id, $action) {
		...
		// Deprecated filters
		$args = apply_filters_deprecated('acfe/form/submit/email/args', [$args, $form, $action], '0.8.1', 'acfe/form/submit/email_args');
		$args = apply_filters_deprecated('acfe/form/submit/email/args/form=' . $form_name, [
			$args,
			$form,
			$action
		], '0.8.1', 'acfe/form/submit/email_args/form=' . $form_name);
		...
		if (!empty($action)) {
			...
			// Deprecated filter
			$args = apply_filters_deprecated('acfe/form/submit/email/args/action=' . $action, [
				$args,
				$form,
				$action
			], '0.8.1', 'acfe/form/submit/email_args/action=' . $action);
	...
	function submit($args, $form, $action) {
		...
		// Deprecated
		$args = apply_filters_deprecated("acfe/form/query_var/email", [$args, $form, $action], '0.8.7.5', "acfe/form/output/email");
		$args = apply_filters_deprecated("acfe/form/query_var/email/form={$form_name}", [
			$args,
			$form,
			$action
		], '0.8.7.5', "acfe/form/output/email/form={$form_name}");
		$args = apply_filters_deprecated("acfe/form/query_var/email/action={$action}", [
			$args,
			$form,
			$action
		], '0.8.7.5', "acfe/form/output/email/action={$action}");
