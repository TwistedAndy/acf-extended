			...
			foreach ($rules as $rule) {
				...
				foreach ($acf_meta as $acf) {
					...
					if (preg_match('/^' . $prefix . '_([0-9]+)_' . $rule['group'] . '$/', $acf['key'], $match)) {
						...
						$updates[$rule['new_field']][$match[1]]['group'] = [
							'key' => $acf['key'],
							'value' => $acf['value'],
						];
						...
						$updates[$rule['new_field']][$match[1]]['sub_field'] = [
							'key' => $acf['key'],
							'value' => $acf['value'],
						];
						...
						$updates[$rule['new_field']][$match[1]]['sub_field_custom'] = [
							'key' => $acf['key'],
							'value' => $acf['value'],
						];
						...
						if (!$load_values) {
							...
							foreach ($fields as $field_key => $field_value) {
								...
								if ($field_key === 'field_acfe_form_post_save_post_content') {
									...
									$update['field_acfe_form_post_save_post_content_group'] = [
						...
						if (!$load_values) {
							...
							foreach ($fields as $field_key => $field_value) {
								...
								if ($field_key === 'field_acfe_form_term_save_description') {
									...
									$update['field_acfe_form_term_save_description_group'] = [
						...
						if (!$load_values) {
							...
							foreach ($fields as $field_key => $field_value) {
								...
								if ($field_key === 'field_acfe_form_user_save_description') {
									...
									$update['field_acfe_form_user_save_description_group'] = [
