...
class acfe_dev {
	...
	function __construct() {
		...
		// load
		add_action('acfe/load_post', [$this, 'load_post']);

		// add meta boxes
		add_action('acfe/add_post_meta_boxes', [$this, 'add_post_meta_boxes'], 10, 2);
		add_action('acfe/add_posts_meta_boxes', [$this, 'add_posts_meta_boxes']);
		add_action('acfe/add_term_meta_boxes', [$this, 'add_term_meta_boxes'], 10, 2);
		add_action('acfe/add_terms_meta_boxes', [$this, 'add_terms_meta_boxes']);
		add_action('acfe/add_user_meta_boxes', [$this, 'add_user_meta_boxes']);
		add_action('acfe/add_option_meta_boxes', [$this, 'add_option_meta_boxes']);

		add_action('acfe/add_settings_meta_boxes', [$this, 'add_settings_meta_boxes']);
		add_action('acfe/add_attachment_meta_boxes', [$this, 'add_attachment_meta_boxes']);
		add_action('acfe/add_attachments_meta_boxes', [$this, 'add_attachments_meta_boxes']);
		add_action('acfe/add_users_meta_boxes', [$this, 'add_users_meta_boxes']);

		// table render
		add_filter('acfe/dev/meta/columns', [$this, 'meta_columns'], 10, 3);
		add_action('acfe/dev/meta/render_column', [$this, 'meta_render_column'], 10, 4);
	...
	function admin_enqueue_scripts() {
		...
		if (acf_is_screen(['profile-network', 'user-edit-network', 'user-network'])) {
			acf_enqueue_scripts();
		}
	...
	function add_user_meta_boxes($user) {
		...
		// add meta boxes
		$this->add_meta_boxes($post_id, ['profile', 'user-edit']);

		// action
		do_action('acfe/dev/add_meta_boxes', $post_id, ['profile', 'user-edit'], 'user');
	...
	function add_meta_boxes($post_id, $screen) {
		...
		if (!empty($this->wp_meta)) {
			...
			add_meta_box($id, $title, [$this, 'render_meta_box'], $screen, $context, $priority, [
				'table' => 'wp_meta',
				'type' => $this->type,
				'bulk' => $bulk
			]);
	...
	function render_meta_box($post, $metabox) {
		...
		//vars
		$args = $metabox['args'];
		$columns = apply_filters('acfe/dev/meta/columns', [], $args);
			...
			<?php foreach ($this->{$args['table']} as $meta): ?>
				...
				<tr>
					...
					<?php foreach ($columns as $column_name => $column_label): ?>
						...
						$attrs = $column_name === 'checkbox' ? ['scope' => 'row', 'class' => 'check-column'] : ['class' => "col-{$column_name}"];
		...
		<script type="text/javascript">
			if (typeof acf !== 'undefined') {
				acf.newPostbox(<?php echo json_encode(['id' => $metabox['id']]); ?>);
			}
		</script>
		<?php
	...
	function meta_columns($columns, $args) {
		...
		if (current_user_can(acf_get_setting('capability'))) {
			$columns = array_merge(['checkbox' => '<input type="checkbox" />'], $columns);
		}
	...
	function meta_render_column($column_name, $meta, $args) {
		...
		switch ($column_name) {
			...
			case 'name':
			{
				...
				$row_actions = apply_filters('acfe/dev/meta/row_actions', [], $meta, $args);
	...
	function setup_meta($post_id = 0) {
		...
		foreach ($wp_meta as $key => $meta) {
			...
			// found field
			if (is_array($field) and !empty($field['type'])) {
