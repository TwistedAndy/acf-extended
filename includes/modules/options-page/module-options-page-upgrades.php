...
class acfe_module_options_page_upgrades {
	...
	function __construct() {
		...
		add_action('acfe/do_upgrade', [$this, 'upgrade_0_8_9'], 40);
		add_action('acfe/do_upgrade', [$this, 'upgrade_0_8_8'], 30);
		add_action('acfe/do_upgrade', [$this, 'upgrade_0_8_6'], 20);
	...
	function upgrade_0_8_9($db_version) {
		...
		add_action('init', function() {
			...
			foreach ($todo as $post_id) {
				...
				$settings = acfe_get_settings("modules.options_pages.{$name}", []);
	...
	function upgrade_0_8_8($db_version) {
		...
		$old = acfe_get_settings('modules.dynamic_option.data', []);
		$new = acfe_get_settings('modules.options_pages', []);
	...
	function upgrade_0_8_6($db_version) {
		...
		foreach ($posts as $post_id) {
			...
			if ($acfe_dop_name === $post_name) {
				...
				// Get ACFE option
				$option = acfe_get_settings('modules.options_pages', []);
	...
	function upgrade_0_8_5($db_version) {
		...
		$old = get_option('acfe_dynamic_options_pages', []);
		$new = acfe_get_settings('modules.options_pages', []);
