...
class ACFE_AutoSync_Json {
	...
	function __construct() {
		...
		// hooks
		add_action('acf/update_field_group', [$this, 'pre_update_field_group'], 9);
		add_action('acf/untrash_field_group', [$this, 'pre_update_field_group'], 9);
		add_action('acf/update_field_group', [$this, 'post_update_field_group'], 11);
		add_action('acf/untrash_field_group', [$this, 'post_update_field_group'], 11);

		add_action('acf/trash_field_group', [$this, 'pre_delete_field_group'], 9);
		add_action('acf/delete_field_group', [$this, 'pre_delete_field_group'], 9);
		add_action('acf/trash_field_group', [$this, 'post_delete_field_group'], 11);
		add_action('acf/delete_field_group', [$this, 'post_delete_field_group'], 11);

		// override settings
		add_filter('acf/settings/json', [$this, 'override_json'], 5);
		add_filter('acf/settings/save_json', [$this, 'override_json_save'], 5);
		add_filter('acf/settings/load_json', [$this, 'override_json_load'], 5);
	...
	function pre_update_field_group($field_group) {
		...
		if (acfe_has_json_sync($field_group)) {
			...
			add_filter('acf/settings/json', [$this, '__return_false']);
	...
	function post_update_field_group($field_group) {
		...
		if (acfe_has_json_sync($field_group)) {
			...
			// reset json setting
			remove_filter('acf/settings/json', [$this, '__return_false']);
...
function acfe_has_json_sync($item) {
	return in_array('json', (array) acf_maybe_get($item, 'acfe_autosync', []));
}
