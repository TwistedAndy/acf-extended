...
function acfe_get_performance_config($key = '') {
	...
	$config = [
		...
		'post_types' => [],      // allowed post types (all)
		'taxonomies' => [],      // allowed taxonomies (all)
	...
	if ($config['engine'] === 'ultra') {
		...
		$config['post_types'] = acfe_apply_filters_deprecated('acfe/modules/single_meta/post_types', [$config['post_types']], '0.8.9.3', 'acfe/modules/performance/config');
		$config['taxonomies'] = acfe_apply_filters_deprecated('acfe/modules/single_meta/taxonomies', [$config['taxonomies']], '0.8.9.3', 'acfe/modules/performance/config');
		$config['users'] = acfe_apply_filters_deprecated('acfe/modules/single_meta/users', [$config['users']], '0.8.9.3', 'acfe/modules/performance/config');
		$config['options'] = acfe_apply_filters_deprecated('acfe/modules/single_meta/options', [$config['options']], '0.8.9.3', 'acfe/modules/performance/config');
 ...
 * @return mixed
 */
function acfe_do_performance_bypass($func, $args = []) {
...
function acfe_is_object_type_performance_enabled($type, $object) {
	...
	switch ($type) {
		...
		case 'post':
		{
			...
			// vars
			$restricted = acfe_get_setting('reserved_post_types', []);
		...
		case 'term':
		{
			...
			// vars
			$restricted = acfe_get_setting('reserved_taxonomies', []);
...
function acfe_get_object_performance_other_engines($post_id) {
	...
	return acfe_query_performance_engines(['name' => $name], 'NOT');
