...
class acfe_module_taxonomy_features {
	...
	function __construct() {
		...
		add_action('acfe/module/register_item/module=taxonomy', [$this, 'register_taxonomy']);
		add_action('admin_footer-edit-tags.php', [$this, 'admin_footer']);
		add_filter('get_terms_args', [$this, 'admin_archive_posts'], 9, 2); // use 9 to let developers use 10 to override
	...
	function register_taxonomy($item) {
		...
		add_filter("edit_{$item['name']}_per_page", [$this, 'admin_archive_ppp']);
	...
	function front_template($template) {
		...
		foreach (get_taxonomies(['public' => true], 'objects') as $taxonomy) {
			...
			if ($locate = locate_template([$taxonomy->acfe_single_template])) {
				return $locate;
			}
