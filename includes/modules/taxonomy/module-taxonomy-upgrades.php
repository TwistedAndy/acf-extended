...
class acfe_module_taxonomy_upgrades {
	...
	function upgrade_0_8_9($db_version) {
		...
		add_action('init', function() {
			...
			foreach ($todo as $post_id) {
				...
				$settings = acfe_get_settings("modules.taxonomies.{$name}", []);
	...
	function upgrade_0_8_8($db_version) {
		...
		$old = acfe_get_settings('modules.dynamic_taxonomy.data', []);
		$new = acfe_get_settings('modules.taxonomies', []);
	...
	function upgrade_0_8_5($db_version) {
		...
		$old = get_option('acfe_dynamic_taxonomies', []);
		$new = acfe_get_settings('modules.taxonomies', []);
