...
class acfe_module_post_type_upgrades {
	...
	function upgrade_0_8_9($db_version) {
		...
		add_action('init', function() {
			...
			foreach ($todo as $post_id) {
				...
				$settings = acfe_get_settings("modules.post_types.{$name}", []);
	...
	function upgrade_0_8_8($db_version) {
		...
		$old = acfe_get_settings('modules.dynamic_post_type.data', []);
		$new = acfe_get_settings('modules.post_types', []);
	...
	function upgrade_0_8_5($db_version) {
		...
		$old = get_option('acfe_dynamic_post_types', []);
		$new = acfe_get_settings('modules.post_types', []);
