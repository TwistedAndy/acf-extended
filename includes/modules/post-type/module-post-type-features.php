...
class acfe_module_post_type_features {
	...
	function __construct() {
		...
		add_action('admin_footer-edit.php', [$this, 'admin_footer']);
		add_action('pre_get_posts', [$this, 'admin_archive_posts'], 9); // use 9 to let developers use 10 to override
		add_filter('edit_posts_per_page', [$this, 'admin_archive_ppp'], 10, 2);
		add_action('pre_get_posts', [$this, 'front_archive_posts'], 9); // use 9 to let developers use 10 to override
	...
	function front_template($template) {
		...
		foreach (get_post_types([], 'objects') as $post_type) {
			...
			// Archive
			if ($rule['has_archive'] && $rule['is_archive'] && $acfe_archive_template && ($locate = locate_template([$post_type->acfe_archive_template]))) {
				return $locate;
			} // Single
			elseif ($rule['is_single'] && $acfe_single_template && ($locate = locate_template([$post_type->acfe_single_template]))) {
				return $locate;
			}
	...
	function post_updated_messages($messages) {
		...
		$scheduled_date = sprintf(__('%1$s at %2$s'), date_i18n(_x('M j, Y', 'publish box date format'), strtotime($post->post_date)), date_i18n(_x('H:i', 'publish box time format'), strtotime($post->post_date)));
