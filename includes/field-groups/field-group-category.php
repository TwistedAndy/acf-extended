...
class acfe_field_group_category {
	...
	function __construct() {
		...
		add_filter('manage_edit-acf-field-group_columns', [$this, 'columns'], 11);
		add_action('manage_acf-field-group_posts_custom_column', [$this, 'column_html'], 10, 2);
		add_filter('views_edit-acf-field-group', [$this, 'views'], 9);
		add_filter('acf/get_taxonomies', [$this, 'acf_get_taxonomies'], 10, 2);

		add_filter('acf/prepare_field_group_for_export', [$this, 'prepare_field_group_for_export']);
		add_action('acf/import_field_group', [$this, 'import_field_group']);
	...
	function init() {
		...
		register_taxonomy('acf-field-group-category', ['acf-field-group'], [
			'hierarchical' => true,
			'public' => false,
			'show_ui' => 'ACFE',
	...
	function import_field_group($field_group) {
		...
		foreach ($categories as $term_slug => $term_name) {
			...
			if (empty($get_term)) {
				...
				$new_term = wp_insert_term($term_name, 'acf-field-group-category', [
					'slug' => $term_slug
				]);
			...
			if ($new_term_id) {
				wp_set_post_terms($field_group['ID'], [$new_term_id], 'acf-field-group-category', true);
			}
