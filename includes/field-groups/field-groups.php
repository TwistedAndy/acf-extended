...
class ACFE_Field_Groups {
	...
	function __construct() {
		...
		add_action('acf/add_meta_boxes', [$this, 'seamless_metabox'], 10, 3);
	...
	function current_screen() {
		...
		// hooks
		add_filter('manage_edit-acf-field-group_columns', [$this, 'table_columns'], 20);
		add_action('manage_acf-field-group_posts_custom_column', [$this, 'table_columns_html'], 10, 2);

		add_filter('default_hidden_columns', [$this, 'default_hidden_columns'], 10, 2);
		add_filter('edit_acf-field-group_per_page', [$this, 'posts_per_page']);
		add_filter('page_row_actions', [$this, 'row_actions'], 20, 2);
		add_filter('display_post_states', [$this, 'post_states'], 20, 2);
		add_action('admin_footer', [$this, 'admin_footer']);

		if ($this->view !== 'sync') {

			add_filter('bulk_actions-edit-acf-field-group', [$this, 'bulk_actions']);
			add_filter('handle_bulk_actions-edit-acf-field-group', [$this, 'handle_bulk_actions'], 10, 3);
	...
	function render_admin_table_column_source_html($field_group) {
		...
		// ACF Extended
		if (in_array($field_group['key'], acfe_get_setting('reserved_field_groups', []))) {
	...
	function render_admin_table_column_php_html($field_group) {
		...
		<span <?php echo acf_esc_atts($wrapper); ?>>
            ...
            <?php if ($return['wrapper_start']) {
	            echo $return['wrapper_start'];
            } ?>
            ...
            <?php if ($return['wrapper_end']) {
	            echo $return['wrapper_end'];
            } ?>
