...
class ACFE_Field_Groups_Local {
	...
	function current_screen() {
		...
		// hooks
		add_filter('views_edit-acf-field-group', [$this, 'views'], 20);
				...
				<tr <?php if ($i % 2 == 0): ?>class="alternate"<?php endif; ?>>
					...
					<td class="post-title page-title column-title">
						...
						<div class="row-actions">
                            ...
                            <span>
                                <a href="<?php echo add_query_arg(['action' => 'php', 'keys' => $key], acf_get_admin_tool_url('acfe-fg-local')); ?>">PHP</a> |
                            </span>

							<span>
                                <a href="<?php echo add_query_arg(['action' => 'json', 'keys' => $key], acf_get_admin_tool_url('acfe-fg-local')); ?>">Json</a> |
                            </span>
							...
							<span>
                                <a href="<?php echo add_query_arg([
	                                'action' => 'sync',
	                                'keys' => $key
                                ], acf_get_admin_tool_url('acfe-fg-local')); ?>">Sync to database</a> |
                            </span>
	...
	function get_local_field_groups() {
		...
		foreach ($local_field_groups as $field_group) {
			...
			if (!acfe_is_super_dev() && in_array($field_group['key'], acfe_get_setting('reserved_field_groups', []))) {
				continue;
			}
