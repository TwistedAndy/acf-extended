...
class acfe_field_choices_label {
	...
	function __construct() {
		...
		// instructions
		add_filter('acf/prepare_field/name=choices', [$this, 'prepare_instructions'], 20);

		// filters
		add_filter('acf/prepare_field/type=radio', [$this, 'prepare_choices'], 20);
		add_filter('acf/prepare_field/type=checkbox', [$this, 'prepare_choices'], 20);
		add_filter('acf/prepare_field/type=acfe_taxonomy_terms', [$this, 'prepare_choices'], 20);

		add_filter('acf/prepare_field/type=radio', [$this, 'prepare_radio'], 20);
		add_filter('acf/prepare_field/type=acfe_taxonomy_terms', [$this, 'prepare_radio'], 20);
	...
	function prepare_instructions($field) {
		...
		// allowed types
		$field_types = ['radio', 'checkbox', 'select'];
	 ...
	 * @return array|mixed
	 */
	function get_labels($choices = [], $depth = 1, $labels = []) {
		...
		foreach ($choices as $key => $value) {
			...
			if (is_array($value)) {
				...
				if (!is_numeric($key)) {
					$labels = array_merge($labels, [$key => $first_key]);
				}
