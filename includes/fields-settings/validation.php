...
class acfe_field_validation {
	...
	function __construct() {
		...
		// actions
		add_action('acf/field_group/admin_head', [$this, 'load']);
		add_action('wp_ajax_acf/field_group/render_field_settings', [$this, 'load_ajax'], 5);

		// filters
		add_filter('acf/validate_value', [$this, 'validate_value'], 99, 4);
	...
	function add_settings() {
		...
		// exclude
		$exclude = ['accordion', 'acfe_button', 'acfe_column', 'acfe_dynamic_render', 'clone', 'flexible_content', 'group', 'message', 'repeater', 'tab'];
		...
		foreach (acf_get_field_types_info() as $field) {
			...
			add_action("acf/render_field_settings/type={$field_type}", [$this, 'render_field_settings'], 99);
	...
	function validate_value($valid, $value, $field, $input) {
		...
		foreach ($field['acfe_validate'] as $k => $rule) {
			...
			foreach ($rule['acfe_validate_rules_and'] as $k => $function) {
				...
				elseif (is_callable([$this, $function['acfe_validate_function']])) {
					$result = call_user_func([$this, $function['acfe_validate_function']], $value);
				} // function
