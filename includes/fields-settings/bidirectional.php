...
class acfe_bidirectional {
	...
	// vars
	var $allowed_types = ['relationship', 'post_object', 'user', 'taxonomy'];
	...
	function __construct() {
		...
		foreach ($this->allowed_types as $allowed_type) {
			...
			add_action("acf/render_field_settings/type={$allowed_type}", [$this, 'field_settings_render']);
			add_filter("acf/update_field/type={$allowed_type}", [$this, 'field_settings_update']);
			add_action("acf/delete_field/type={$allowed_type}", [$this, 'field_settings_delete']);
			add_filter("acf/update_value/type={$allowed_type}", [$this, 'update_value'], 11, 3);

		}

		add_action('wp_ajax_acfe/fields_settings/bidirectional/query', [$this, 'ajax_query']);
		add_action('wp_ajax_nopriv_acfe/fields_settings/bidirectional/query', [$this, 'ajax_query']);

		add_filter('acf/prepare_field/name=acfe_bidirectional_related', [$this, 'field_settings_default_value']);
	...
	function get_related_settings($r_field, $r_field_group, &$choices) {
		...
		if (in_array($r_field['type'], ['repeater', 'flexible_content'])) {
			return;
		}
		...
		foreach ($r_fields as $r_field_key) {
			...
			if (!empty($r_field_ancestors)) {
				...
				for ($i = count($r_values_query) - 1; $i >= 0; $i--) {
					$r_values = [$r_values_query[$i] => $r_values];
				}
