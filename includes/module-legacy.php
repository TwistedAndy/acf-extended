...
// vars
public $name = '', $active = false, $settings = '', $post_type = '', $label = '', $textdomain = '', $tool = '', $columns = [], $tools = [];
...
function __construct() {
	...
	add_action('current_screen', [$this, '_current_screen']);

	add_action('acf/save_post', [$this, '_save_post'], 20);
	add_action('trashed_post', [$this, '_trashed_post']);
	add_action('untrashed_post', [$this, '_untrashed_post']);
	add_filter('acf/get_post_types', [$this, 'get_post_types'], 10, 2);
...
function _current_screen() {
	...
	if (acf_is_screen($this->post_type)) {
		...
		add_action('admin_enqueue_scripts', [$this, '_post_head']);
		add_filter('admin_body_class', [$this, 'post_body_class']);
		add_action('post_submitbox_misc_actions', [$this, '_post_submitbox_misc_actions']);
		add_filter('enter_title_here', [$this, 'post_enter_title_here'], 10, 2);
		add_action('admin_footer', [$this, '_post_footer']);
		add_action('load-post.php', [$this, 'post_load']);
		add_action('load-post-new.php', [$this, 'post_new_load']);
		add_filter('submenu_file', [$this, 'submenu_file']);
		add_filter('acfe/localize_data', [$this, 'post_localize_data']);
		...
		add_action('admin_enqueue_scripts', [$this, '_edit_head']);
		add_filter('admin_body_class', [$this, 'edit_body_class']);
		add_filter("manage_edit-{$this->post_type}_columns", [$this, '_edit_columns']);
		add_action("manage_{$this->post_type}_posts_custom_column", [$this, 'edit_columns_html'], 10, 2);
		add_filter('display_post_states', [$this, 'display_post_states'], 10, 2);
		add_filter('post_row_actions', [$this, '_edit_row_actions'], 10, 2);
		add_filter('page_row_actions', [$this, '_edit_row_actions'], 10, 2);
		add_action('admin_footer', [$this, 'edit_footer']);
		add_action('load-edit.php', [$this, 'edit_load']);
		add_filter("bulk_actions-edit-{$this->post_type}", [$this, 'bulk_actions']);
		add_filter("handle_bulk_actions-edit-{$this->post_type}", [$this, 'handle_bulk_actions'], 10, 3);
		add_filter('acfe/localize_data', [$this, 'edit_localize_data']);
...
<script type="text/javascript">
	...
	function _edit_columns($columns) {
		...
		$columns = array_merge(['cb' => $columns['cb'], 'title' => $columns['title']], $this->columns);
	...
	function _edit_row_actions($actions, $post) {
		...
		if (!in_array($post->post_status, ['publish', 'acf-disabled'])) {
			return $actions;
		}
