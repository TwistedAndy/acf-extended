...
class acfe_location_post_type_list {
	...
	function __construct() {
		...
		add_filter('acf/location/rule_values/post_type_list', [$this, 'rule_values']);
		add_filter('acf/location/rule_match/post_type_list', [$this, 'rule_match'], 10, 3);
	...
	function load_posts($post_type) {
		...
		if (acf_verify_nonce('post_type_list')) {
			...
			if (acf_validate_save_post(true)) {
				...
				// Redirect
				wp_redirect(add_query_arg(['message' => 'post_type_list']));
				exit;
	...
	function add_posts_meta_boxes() {
		...
		foreach ($field_groups as $position => $_field_groups) {
			...
			foreach ($_field_groups as $field_group) {
				...
				// Add the meta box.
				add_meta_box($id, acf_esc_html($title), [$this, 'render_meta_box'], 'edit', $context, $priority, [
