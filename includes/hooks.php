...
class acfe_hooks {
	...
	function __construct() {
		...
		add_action('acf/validate_save_post', [$this, 'validate_save_post'], 4); // must be 4 as acf process acf/validate_value on 5
		...
		// form data
		add_filter('acf/location/screen', [$this, 'acf_location_screen'], 99);
		add_action('acf/input/form_data', [$this, 'acf_form_data']);
		add_action('acf/validate_save_post', [$this, 'acf_save_post_form_data'], 0);
		add_action('acf/save_post', [$this, 'acf_save_post_form_data'], 0);

		// upload
		add_filter('acf/upload_prefilter', [$this, 'attachment_upload'], 10, 3);

		// hooks variations
		acf_add_filter_variations('acfe/prepare_field_group', ['ID', 'key'], 0);
		acf_add_action_variations('acfe/pre_render_field_group', ['ID', 'key'], 0);
		acf_add_action_variations('acfe/render_field_group', ['ID', 'key'], 0);
		acf_add_filter_variations('acf/field_wrapper_attributes', ['type', 'name', 'key'], 1);
		acf_add_filter_variations('acfe/field_wrapper_attributes', ['type', 'name', 'key'], 1);
		acf_add_filter_variations('acfe/load_fields', ['type', 'name', 'key'], 1);
		acf_add_filter_variations('acfe/load_field', ['type', 'name', 'key'], 0);
		acf_add_filter_variations('acfe/upload_dir', ['type', 'name', 'key'], 1);
		acf_add_filter_variations('acfe/upload_file', ['type', 'name', 'key'], 1);
	...
	function acf_location_screen($screen) {
		...
		// clone var
		$_screen = $screen;
		acf_extract_vars($_screen, ['lang', 'ajax']);
	...
	function attachment_upload($errors, $file, $field) {
		...
		// filters
		add_filter('upload_dir', [$this, 'handle_upload_dir'], 20);
		add_filter('wp_handle_upload_prefilter', [$this, 'handle_upload_file'], 20);
