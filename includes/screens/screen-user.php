...
class acfe_screen_user {
	...
	function __construct() {
		...
		// add
		add_action('load-user-new.php', [$this, 'user_new_load']);

		// edit
		add_action('load-profile.php', [$this, 'user_load']);
		add_action('load-user-edit.php', [$this, 'user_load']);

		// list
		add_action('load-users.php', [$this, 'users_load']);
	...
	function user_new_load() {
		...
		// hooks
		add_action('user_new_form', [$this, 'user_new_meta_boxes']);
	...
	function user_load() {
		...
		if (acf_is_screen(['user-edit-network', 'profile-network'])) {
			return;
		}
		...
		// hooks
		add_action('show_user_profile', [$this, 'user_meta_boxes']);
		add_action('edit_user_profile', [$this, 'user_meta_boxes']);
	...
	function users_load() {
		...
		// do not process on network screens
		if (acf_is_screen(['users-network'])) {
			return;
		}
		...
		// hooks
		add_action('admin_footer', [$this, 'users_footer']);
