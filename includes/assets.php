...
class acfe_assets {
	...
	function __construct() {
		...
		add_action('acf/admin_enqueue_scripts', [$this, 'acf_admin_enqueue_scripts']);
		add_action('acf/input/admin_enqueue_scripts', [$this, 'acf_input_admin_enqueue_scripts']);
		add_action('acf/enqueue_scripts', [$this, 'acf_enqueue_scripts'], 99);
	...
	function init() {
		...
		// register scripts
		wp_register_script('acf-extended', acfe_get_url("assets/js/acfe{$min}.js"), ['acf'], $version);
		wp_register_script('acf-extended-input', acfe_get_url("assets/js/acfe-input{$min}.js"), ['acf-extended', 'acf-input'], $version);
		wp_register_script('acf-extended-admin', acfe_get_url("assets/js/acfe-admin{$min}.js"), ['acf-extended'], $version);
		wp_register_script('acf-extended-field-group', acfe_get_url("assets/js/acfe-field-group{$min}.js"), ['acf-extended', 'acf-field-group'], $version);
		wp_register_script('acf-extended-ui', acfe_get_url("assets/js/acfe-ui{$min}.js"), ['acf-extended'], $version);

		// register styles
		wp_register_style('acf-extended', acfe_get_url("assets/css/acfe{$min}.css"), [], $version);
		wp_register_style('acf-extended-input', acfe_get_url("assets/css/acfe-input{$min}.css"), [], $version);
		wp_register_style('acf-extended-admin', acfe_get_url("assets/css/acfe-admin{$min}.css"), [], $version);
		wp_register_style('acf-extended-field-group', acfe_get_url("assets/css/acfe-field-group{$min}.css"), [], $version);
		wp_register_style('acf-extended-ui', acfe_get_url("assets/css/acfe-ui{$min}.css"), [], $version);
	...
	function acf_enqueue_scripts() {
		...
		// text
		$text = apply_filters('acfe/localize_text', [
			'Close' => __('Close', 'acfe'),
		...
		// data
		$data = apply_filters('acfe/localize_data', [
			'version' => ACFE_VERSION,
			'home_url' => home_url(),
			'is_admin' => is_admin(),
...
function acfe_localize_data() {
	acf_localize_data(['acfe' => acfe_get_localize_data()]);
}
