 ...
 * @return false|mixed
 */
function acfe_query_field($args = []) {
 ...
 * @return array
 */
function acfe_query_fields($args = []) {
	...
	// validate query
	$args = wp_parse_args($args, [
		'query' => [],                  // main query, should be compatible with wp_list_filter()
	...
	if (acf_is_associative_array($args['context'])) {
		...
		if (acf_is_field_group($args['context'])) {
			...
			foreach ($args['_query'] as $q) {
				$storage = array_merge($storage, wp_list_filter([$args['context']], $q[0], $q['relation']));
			}
		...
		if ($fields) {
			foreach ($fields as $field) {
				...
				foreach ($args['_query'] as $q) {
					$storage = array_merge($storage, wp_list_filter([$field], $q[0], $q['relation']));
				}
